...
import { getCustomImageUrl, hasCustomImage } from "./custom_images";
...
export class AssetsManager {
  ...
  async getImage(id: number, options: GetImageOptions = {}): Promise<Blob> {
    ...
    if (hasCustomImage(id)) {
      const customUrl = await getCustomImageUrl(id);
      if (customUrl) {
        try {
          const response = await fetch(customUrl);
          return await response.blob();
        } catch (error) {
          console.error(`Failed to fetch custom image for ID ${id}:`, error);
          // 发生错误时继续使用默认图像
        }
      }
    }
